...
var expTable = [256]byte{1, 3, 5, 15, 17, 51, 85, 255, 26, 46, 114,
...
var logTable = [256]int{0, 0, 25, 1, 50, 2, 26, 198, 75, 199, 27, 104,
...
func mul(lhs, rhs byte) byte {
	// add the logarithms, then raise back up, but avoid zero!
	if lhs == 0 || rhs == 0 {
		return 0
	}
	return expTable[(logTable[lhs]+logTable[rhs])%255]
}

func div(dividend, divisor byte) byte {
	a, b := logTable[dividend], logTable[divisor]
	if b > a {
		return expTable[255+a-b]
	}
	return expTable[a-b]
}
