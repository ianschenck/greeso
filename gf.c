...
uint8_t exp_table[256] = {
  1, 3, 5, 15, 17, 51, 85, 255, 26, 46, 114,
...
int log_table[256] = {
  0, 0, 25, 1, 50, 2, 26, 198, 75, 199, 27, 104,
  51, 238, 223, 3, 100, 4, 224, 14, 52, 141, 129, 239, 76, 113, 8,
  200, 248, 105, 28, 193, 125, 194, 29, 181, 249, 185, 39, 106, 77,
  228, 166, 114, 154, 201, 9, 120, 101, 47, 138, 5, 33, 15, 225, 36,
  18, 240, 130, 69, 53, 147, 218, 142, 150, 143, 219, 189, 54, 208,
  206, 148, 19, 92, 210, 241, 64, 70, 131, 56, 102, 221, 253, 48,
  191, 6, 139, 98, 179, 37, 226, 152, 34, 136, 145, 16, 126, 110, 72,
  195, 163, 182, 30, 66, 58, 107, 40, 84, 250, 133, 61, 186, 43, 121,
  10, 21, 155, 159, 94, 202, 78, 212, 172, 229, 243, 115, 167, 87,
  175, 88, 168, 80, 244, 234, 214, 116, 79, 174, 233, 213, 231, 230,
  173, 232, 44, 215, 117, 122, 235, 22, 11, 245, 89, 203, 95, 176,
  156, 169, 81, 160, 127, 12, 246, 111, 23, 196, 73, 236, 216, 67,
  31, 45, 164, 118, 123, 183, 204, 187, 62, 90, 251, 96, 177, 134,
  59, 82, 161, 108, 170, 85, 41, 157, 151, 178, 135, 144, 97, 190,
...
extern uint8_t gf_add(uint8_t lhs, uint8_t rhs) {
  return lhs ^ rhs;
}

extern uint8_t gf_sub(uint8_t lhs, uint8_t rhs) {
  return lhs ^ rhs;
}

extern uint8_t gf_mul(uint8_t lhs, uint8_t rhs) {
  if (lhs == 0 || rhs == 0) {
	 return 0;
  }
  return exp_table[(log_table[lhs]+log_table[rhs])%255];
}

extern uint8_t gf_div(uint8_t dividend, uint8_t divisor) {
  int a = log_table[dividend];
  int b = log_table[divisor];
  if (b > a) {
	 return exp_table[255+a-b];
  }
